        ...
        <div className="row justify-content-center">
          <div className="col-xl-10">
            <div className="support-media-detail-head mb-30">
              <Link href={`/${locale}/support/${item.type}`} className="support-media-card-link">
                Назад к списку
              </Link>
              <div className="support-media-meta mt-15">
                ...
                {item.fileType && item.fileSize && <span>{item.fileType} • {item.fileSize}</span>}
              </div>
              <h1 className="support-media-title mt-20">{item.title}</h1>
            </div>

            <div className="support-media-detail-cover mb-30">
              <Image src={item.coverImage} alt={item.title} width={1100} height={620} priority />
            </div>
            ...
            {item.type === "photo" && Array.isArray(item.gallery) && item.gallery.length > 0 && (
              <div className="row">
                {item.gallery.map((imagePath, index) => (
                  <div key={`${imagePath}-${index}`} className="col-lg-4 col-md-6 mb-20">
            ...
            {item.type === "video" && item.sourceUrl && (
              <div className="support-media-detail-cta">
                <a href={item.sourceUrl} target="_blank" rel="noreferrer" className="tp-btn-border">
